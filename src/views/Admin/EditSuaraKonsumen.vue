            ...
            <div class="max-w-6xl mx-auto bg-white p-6 mt-6 rounded-lg shadow-md">
                ...
                <form @submit.prevent="UpdateBerita" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.deskripsi3" id="deskripsi2"
                            class="w-full p-3 border rounded-md bg-white resize-none focus:ring focus:ring-blue-300"
                            placeholder="Tambahkan deskripsi tambahan (jika ada)..." rows="4"></textarea>
                    </div>
                    <!-- Tanggal Publikasi -->
...
export default {
    ...
    data() {
        return {
            ...
            lastIndexImage: [],
            DataBerita: {},
            idBerita: this.$route.query.id || this.$route.params.id, // Ambil token dari query atau path
        };
    },
    mounted() {
        this.fetchWilayah()
        this.getBeritaByid(this.idBerita)
    },
    computed: {
        isSidebarOpen() {
    ...
    methods: {
        ...
        getBeritaByid(id) {
            api.getSuaraKonsumenByid(id).then((res) => {
        ...
        async UpdateBerita() {
            ...
            await api.UpdateSuaraKonsumen(this.idBerita, this.form).then(() => {
                this.$toast.success('Suara Konsumen Berhasil Di Update')
                setTimeout(() => {
                    this.$router.push('/admin/daftar-suara-konsumen')
                }, 1000);
            }).catch(() => {
                this.$toast.error('Suara Konsumen gagal Di Update')
            })

<template>
  <div class="w-screen bg-gray-100 min-h-screen h-full flex">
    <NavbarAnggota />
    <div class="w-full min-h-screen bg-gray-100 flex justify-center items-center mt-6 mr-8">
      <div class="w-full max-w-md sm:max-w-2xl bg-white p-6 mt-12 mb-8 rounded-lg shadow-xl">
        <section class="text-center mb-8 py-8 bg-red-600 rounded-lg shadow-xl flex flex-col h-full">
          ...
          <div class="space-y-4 p-6">
            <div class="flex flex-col sm:flex-row sm:items-center">
              <label class="block text-sm font-medium text-white sm:mr-3">
                No Rekening: <strong>{{ banks[0].accountNumber }}</strong>
              </label>
              <button @click="copyToClipboard(banks[0].accountNumber)"
                class="text-gray-500 hover:text-gray-700 transition-all duration-200 ease-in-out sm:ml-2">
        ...
        <section class="space-y-6 mb-6">
          <div class="space-y-4">
            ...
            <div class="relative mt-2">
              ...
              <div v-if="fileName" class="mt-2 text-gray-700">
            ...
            <div class="border-b-2 pt-2 pb-2 p-2">
              ...
              <div class="relative mt-2">
                <input v-model="jumlahPembayaranDisplay" @input="formatInputAmount"
                  class="w-full p-3 bg-white border font-semibold border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500"
                  type="text" placeholder="Rp." required />
              </div>
            </div>
          </div>
        </section>
        <div v-if="selectedPaymentMethod" class="flex justify-center">
          <button type="submit" @click="submitForm()"
            class="w-full sm:w-auto bg-red-500 text-white px-6 py-2 rounded-lg font-semibold hover:bg-red-600 transition duration-300 ease-in-out">
            Bayar Sekarang
          </button>
        </div>
      </div>

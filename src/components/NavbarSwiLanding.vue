<template>
    <div class="relative flex">
        ...
        <button v-if="!sidebarOpen || isDesktop" @click="toggleSidebar" type="button"
            class="fixed top-4 left-4 z-50 p-4 text-gray-800 rounded-lg sm:left-64 sm:hidden transition-transform duration-300 ease-in-out">
        ...
        <transition name="slide">
            <aside v-if="sidebarOpen"
                class="fixed top-0 left-0 z-40 w-64 h-screen bg-emerald-600 text-white shadow-lg flex flex-col sm:translate-x-0 transition-transform duration-300 ease-in-out">
                ...
                <div class="px-4 py-5 bg-emerald-600 flex items-center justify-between">
                    ...
                    <button @click="toggleSidebar"
                        class="text-white text-2xl sm:hidden transition-transform duration-300 ease-in-out">
        ...
        <transition name="fade">
            <div v-if="sidebarOpen && !isDesktop" @click="toggleSidebar"
                class="fixed inset-0 bg-black opacity-50 z-30 sm:hidden transition-opacity duration-300 ease-in-out">
            </div>
        </transition>
